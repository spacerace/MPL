        ...
        for( i = 0, mouse_moved = FALSE; i < numbuttons; i++)
            ...
            if(MouButtonPressed(MouEvent,buttons[i].mask))
                ...
                if(!buttons[i].down)            // if button was previously up
                    {
                    MsgQSend(qhandle,&MouEvent,sizeof(MouEvent),buttons[i].downmsg);
                    buttons[i].down = TRUE;
                    }
                ...
                if(buttons[i].down)             // if button previously down
                    {
                    MsgQSend(qhandle,&MouEvent,sizeof(MouEvent),buttons[i].upmsg);
                    buttons[i].down = FALSE;
                    }
                }
    // notify of all mouse movement if button is down (PM gets all movement)
        if(mouse_moved)
