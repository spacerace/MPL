...
void far pascal handle_signals(int, int); /* BREAK signals handler */
...
unsigned	FileHndl = NULL;    /* COM port file handle */
...
char 	*argv[];
{
	unsigned char	*Stack1; 	     /* stack for a thread */
	unsigned	ThreadID,
			RetCode,	     /* return code */
	...
	long		PrevAddress;
	unsigned	PrevAction;
	static struct KbdStatus OurKbdStatus = 
	...
	if ((RetCode = DOSEXITLIST(ADD_ADDR, xit)) != 0)
	...
	if (Result) {
	  ...
	  if ((RetCode = KBDSETSTATUS((struct KbdStatus *) (&OurKbdStatus), 
                                      RESERVED)) != 0) 
...
/***	init_com_port - open the COM port and initialise line charecteristics
 *
...
void init_com_port()
{
	unsigned 	ActionTaken, /* action: file existed,created,replaced */
		 	RetCode;
	...
	/* set the baud rate */
	if ((RetCode = DOSDEVIOCTL(0L, (char *) &(sComOptions.iBaudRate), 
	...
	if ((RetCode = DOSDEVIOCTL(0L, (char *) &sLineChar, SETLINECHAR, 
			           SERIAL, FileHndl)) != 0)
	  error(ERR_IOCTLSETLINECHAR, RetCode);

	/* get device control block info */
	if ((RetCode = DOSDEVIOCTL((char *) &sDCB, 0L, GETDCB, SERIAL, 
                                   FileHndl)) != 0)
	...
	if ((RetCode = DOSDEVIOCTL(0L, (char *) &sDCB, SETDCB, SERIAL, 
				    FileHndl)) != 0)
...
void write_com_port()
{
	unsigned	NumBytes,	/* number of bytes actually written */
			RetCode;
	char		OutBuffer;	/* output buffer */
	struct KeyData	OurKeyData;   	/* struc to read a char from kbd */


	while (!ExitPgm) {
	  /* read input from the keyboard */
	  if ((RetCode = KBDCHARIN((struct KeyData *) (&OurKeyData), 
                                   IOWAIT, RESERVED)) != 0) 
            error(ERR_KBDCHARIN, RetCode);
	  OutBuffer = OurKeyData.char_code;
	  if ((OutBuffer == 0) || (OutBuffer == 0xE0)) {
	    OutBuffer = OurKeyData.scan_code;
	    switch (OutBuffer) {
...
{
	unsigned	NumBytes,	/* number of bytes actually read */
			RetCode;
	...
	while (!ExitPgm) {
	  ...
	  if ((RetCode = DOSDEVIOCTL((char *) &sCharsInQue, 0L, GETNUMCHARS,
                                     SERIAL, FileHndl)) != 0)
...
void far pascal handle_signals(SigArg, SigNumber)
int 	SigArg,
	SigNumber;
{
	unsigned 	RetCode,
	...
	case BREAK :/* send BREAK to the com port */
	  	    if ((RetCode = DOSDEVIOCTL((char *) &Status, 0L, 
