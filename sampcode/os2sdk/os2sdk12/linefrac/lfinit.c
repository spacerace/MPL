...
LfInitApp(VOID)
{
    ...
    if (!((global.hptr)[0]   = WinLoadPointer( HWND_DESKTOP,(HMODULE) NULL, IDR_KOCH0 )))
	return FALSE;
    if (!((global.hptr)[1]   = WinLoadPointer( HWND_DESKTOP,(HMODULE) NULL, IDR_KOCH1 )))
	return FALSE;
    if (!((global.hptr)[2]   = WinLoadPointer( HWND_DESKTOP,(HMODULE) NULL, IDR_KOCH2 )))
	return FALSE;
    if (!((global.hptr)[3]   = WinLoadPointer( HWND_DESKTOP,(HMODULE) NULL, IDR_KOCH3 )))
	return FALSE;
    if (!(global.hptrTrack   = WinLoadPointer( HWND_DESKTOP,(HMODULE) NULL, IDR_TRACKINGPTR )))
	return FALSE;
    if (!(global.hptrPaste   = WinLoadPointer( HWND_DESKTOP,(HMODULE) NULL, IDR_PASTINGPTR )))
	return FALSE;
