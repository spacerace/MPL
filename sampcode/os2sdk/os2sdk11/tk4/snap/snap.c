...
/* Function prototypes */
MRESULT CALLBACK SnapWndProc(HWND, USHORT, MPARAM, MPARAM);
MRESULT CALLBACK AboutWndProc(HWND, USHORT, MPARAM, MPARAM);
MRESULT CALLBACK SaveFileDlgProc(HWND, USHORT, MPARAM, MPARAM);
...
int cdecl main(void)
{
    ...
    while (WinGetMsg(habSnap, (PQMSG)&msg, NULL, NULL, NULL)) {
...
BOOL SnapInit(void)
{
     ...
     * Initialize the HAB
     */
    habSnap = WinInitialize(NULL);

    hmqSnap = WinCreateMsgQueue(NULL, 0);

    WinLoadString(habSnap, (ULONG)0, IDS_SNAP, sizeof(szSnap), (PCH)szSnap);
...
MRESULT CALLBACK SnapWndProc(hwnd, message, mp1, mp2)
HWND    hwnd;
USHORT  message;
MPARAM  mp1;
MPARAM  mp2;
{
    ...
    switch (message) {
        ...
        case WM_COMMAND:
			...
			(PFNWP)AboutWndProc, NULL, IDD_INFO, (PCH)NULL);
                    break;
            }
            break;
...
HPS hps;
{
    ...
    if (hbm = WinQueryClipbrdData(habSnap, CF_BITMAP)) {
