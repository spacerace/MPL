...
#define ErrMsg(h, s) \
    WinMessageBox(HWND_DESKTOP, h, s, NULL, NULL, MB_OK | MB_ICONEXCLAMATION)

char Ctrl(char ch) {
    return (('a' <= ch) && (ch <= 'z')) ? (ch - 'a' + '\001') :
     ...
     switch (msg) {
	  ...
	  case WM_MSGBOX:
		...
		switch ((int) mp1) {
		    ...
		    default:
			ErrMsg(hWnd, aszMessage[(int) mp1]);
			break;
		}
...
*/
	...
	case WM_CONTROL:
	    ...
	    */
	    ...
	    } else if InRange(bTemp, IDD_FIVE, IDD_EIGHT) {
		comTemp.bData = bTemp;
	    } else if InRange(bTemp, IDD_ONESTOP, IDD_TWOSTOP) {
		comTemp.bStop = bTemp;
	    } else switch (bTemp) {
...
*/
	...
	MIT_END, MIS_SEPARATOR, 0x0000, NULL, NULL, NULL,
	MIT_END, MIS_TEXT, 0x0000, IDM_ABOUT, NULL, NULL
    };
    ...
    */
    ...
    idSM = (SHORT) WinSendMsg(hSM, MM_ITEMIDFROMPOSITION, NULL, NULL);
...
*/
	...
	case WM_QUERYTRACKINFO:
	    rc = (BOOL) (*pfnOldFrameWndProc)(hWnd, msg, mp1, mp2);
